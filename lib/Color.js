...
Color["prototype"] = Object.create(Color, {
    "constructor":  { "value": Color          }, // new Color(value:Color|String|NumberArray|Uint8Array|Uint8ClampedArray|UINT32 = 0x000000):Color
    "toString":     { "value": Color_toString }, // Color#toString(format:String = "rgba"):String
...
function Color_toString(format) { // @arg String = "rgba" - "rgba", "hsva", "hsla", "#rrggbb"
                                  // @ret String - "rgba(255,255,255,1.00)" or "#FFFFFF"
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(format, "String|omit"),            Color_toString, "format");
        $valid($some(format, "rgba|hsva|hsla|#rrggbb"), Color_toString, "format");
    }
//}@dev
    ...
    switch (format || "rgba") {
    case "rgba": return "rgba(" + c[0] + "," + c[1] + "," + c[2] + "," + (c[3] / 255).toFixed(2) + ")";
    case "hsla": return "hsla(" + _hsx2str(Color_RGBA_HSLA(this._c)).join(",") + ")";
    case "hsva": return "hsva(" + _hsx2str(Color_RGBA_HSVA(this._c)).join(",") + ")";
    }
    var n = (c[0] << 16 | c[1] << 8 | c[2]);
    return "#" + (0x1000000 + n).toString(16).slice(1);

    function _hsx2str(f32) {
        var a = [].slice.call(f32);
        return [ a[0], ((a[1] * 100) | 0) + "%",
                       ((a[2] * 100) | 0) + "%", (a[3] / 255).toFixed(2) ];
    }
}
...
//}@dev
    ...
    for (var i = 0, iz = result.length; i < iz; i += 4) {
        ...
        result.set([(r + m) * 255, (g + m) * 255, (b + m) * 255, A], i);
    }
    return result;
}
...
//}@dev
    ...
    if (Color["LOW_PRECISION"]) {
        for (var i = 0, iz = result.length; i < iz; i += 4) {
            ...
            var y  = ( 299 * r + 587 * g + 114 * b) / 255000; // 255 * 1000
            var cb = (-169 * r - 331 * g + 500 * b) / 255000;
            var cr = ( 500 * r - 419 * g -  81 * b) / 255000;
...
function Color_effect(pixelData, // @arg NumberArray|Uint8Array|Uint8ClampedArray - pixel data. [<R, G, B, A>, ...]
                      hslA) {    // @arg NumberArray|Float32Array - effect values. [<h, s, l, A>] [-359..359, -1..1, -1..1, -255..255]
                                 // @ret Uint8ClampedArray - new pixel data. [<R, G, B, A>, ...]
//{@dev
    if (!global["BENCHMARK"]) {
