...
// --- class / interfaces ----------------------------------
function Color(value) { // @arg Color|String|Uint8Array|Uint8ClampedArray|UINT32 = 0x000000 - Color or "#ffffff" or "rgba(,,,)" or "hsla(,,,)" or "hsva(,,,)" or "red" or 0xffffff or [r, g, b, a]
    var v = value;

//{@dev
    $valid($type(v, "Color|String|Uint8Array|Uint8ClampedArray|UINT32|omit"), Color, "value");
    if (v) {
...
//}@dev
    ...
    this._c = v === undefined                ? new Uint8ClampedArray(4)
            : v instanceof Uint8Array        ? new Uint8ClampedArray(v)
            : v instanceof Uint8ClampedArray ? new Uint8ClampedArray(v)
            : v instanceof Color             ? new Uint8ClampedArray([v["R"], v["G"], v["B"], v["A"]])
...
// --- converter ---
Color["RGBA_HSLA"] = Color_RGBA_HSLA; // Color.RGBA_HSLA(RGBA:Uint8ClampedArray):Float32Array - [<h, s, l, A>, ...]
Color["RGBA_HSVA"] = Color_RGBA_HSVA; // Color.RGBA_HSVA(RGBA:Uint8ClampedArray):Float32Array - [<h, s, v, A>, ...]
Color["RGBA_YUVA"] = Color_RGBA_YUVA; // Color.RGBA_YUVA(RGBA:Uint8ClampedArray):Float32Array - [<y, cb, cr, A>, ...]

Color["HSLA_RGBA"] = Color_HSLA_RGBA; // Color.HSLA_RGBA(hslA:Float32Array):Uint8ClampedArray - [R, G, B, A]
Color["HSVA_RGBA"] = Color_HSVA_RGBA; // Color.HSVA_RGBA(hsvA:Float32Array):Uint8ClampedArray - [R, G, B, A]
Color["YUVA_RGBA"] = Color_YUVA_RGBA; // Color.YUVA_RGBA(yuvA:Float32Array):Uint8ClampedArray - [R, G, B, A]

// --- effect ---
Color["reverse"]   = Color_reverse;   // Color.reverse(pixelData:Uint8ClampedArray):Uint8ClampedArray
Color["effect"]    = Color_effect;    // Color.effect(pixelData:Uint8ClampedArray, hslA:NumberArray|Float32Array):Uint8ClampedArray
Color["sepia"]     = Color_sepia;     // Color.sepia(pixelData:Uint8ClampedArray, cb:Number = -0.091, cr:Number = 0.056):Uint8ClampedArray
Color["gray"]      = Color_gray;      // Color.gray(pixelData:Uint8ClampedArray):Uint8ClampedArray
...
function _parseColorString(value) { // @arg String - "#ffffff" or "rgba(,,,)" or "hsla(,,,)" or "hsva(,,,)" or "red"
    ...
    if (v[0] === "#") {
        ...
        if (type) {
            ...
            case 2: return Color_HSLA_RGBA(new Float32Array([R, G / 100, B / 100, A]));
            case 3: return Color_HSVA_RGBA(new Float32Array([R, G / 100, B / 100, A]));
            }
        }
...
// === HSLA ================================================
function Color_HSLA_RGBA(hslA) { // @arg Float32Array - [<h, s, l, A>, ...] [0..359, 0..1, 0..1, 0..255]
                                 // @ret Uint8ClampedArray - [<R, G, B, A>, ...]
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(hslA, "Float32Array"), Color_HSLA_RGBA, "hslA");
    }
//}@dev
    // http://www.rapidtables.com/convert/color/hsl-to-rgb.htm
    // http://gyazo.com/340bb26d09ac9047a092b7416f2fbe74.png

    var result = new Uint8ClampedArray(hslA.length);
...
function Color_RGBA_HSLA(RGBA) { // @arg Uint8ClampedArray - [<R, G, B, A>, ...]
                                 // @ret Float32Array - [<h, s, l, A>, ...] <0..359, 0..1, 0..1, 0..255>
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(RGBA, "Uint8ClampedArray"), Color_RGBA_HSLA, "RGBA");
    }
//}@dev
    // http://www.rapidtables.com/convert/color/rgb-to-hsl.htm
    // http://gyazo.com/24f284e3a85edeb6f3c197386131db73.png

    var result = new Float32Array(RGBA.length);

    for (var i = 0, iz = result.length; i < iz; i += 4) {
        var r   = RGBA[i    ] / 255; // R'
        var g   = RGBA[i + 1] / 255; // G'
        var b   = RGBA[i + 2] / 255; // B'
        var A   = RGBA[i + 3];
...
// === HSVA ================================================
function Color_HSVA_RGBA(hsvA) { // @arg Float32Array - [<h, s, v, A>, ...] [0..359, 0..1, 0..1, 0..255]
                                 // @ret Uint8ClampedArray - [<R, G, B, A>, ...]
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(hsvA, "Float32Array"), Color_HSVA_RGBA, "hsvA");
    }
//}@dev
    // http://www.rapidtables.com/convert/color/hsv-to-rgb.htm
    // http://gyazo.com/89cfc85e8f429638c16336f954c01a6e.png

    var result = new Uint8ClampedArray(hsvA.length);
...
function Color_RGBA_HSVA(RGBA) { // @arg Uint8ClampedArray - [<R, G, B, A>, ...]
                                 // @ret Float32Array - [<h, s, v, A>, ...] <0..359, 0..1, 0..1, 0..255>
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(RGBA, "Uint8ClampedArray"), Color_RGBA_HSVA, "RGBA");
    }
//}@dev
...
// === YUV ================================================
function Color_YUVA_RGBA(yuvA) { // @arg Float32Array - [<y, cb, cr, A>, ...] <0..1, -0.5..0.5, -0.5..0.5, 0..255>
                                 // @ret Uint8ClampedArray - [<R, G, B, A>, ...]
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(yuvA, "Float32Array"), Color_YUVA_RGBA, "yuvA");
    }
//}@dev
    // ITU-R BT.601

    var result = new Uint8ClampedArray(yuvA.length);
...
function Color_RGBA_YUVA(RGBA) { // @arg Uint8ClampedArray - [<R, G, B, A>, ...]
                                 // @ret Float32Array - [<y, cb, cr, A>, ...] [0..1, -0.5..0.5, -0.5..0.5, 0..255]
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(RGBA, "Uint8ClampedArray"), Color_RGBA_YUVA, "RGBA");
    }
//}@dev
    // ITU-R BT.601
...
// === EFFECT ==============================================
function Color_reverse(pixelData) { // @arg Uint8ClampedArray - pixel data. [<R, G, B, A>, ...]
                                    // @ret Uint8ClampedArray - new pixel data. [<R, G, B, A>, ...]
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(pixelData, "Uint8ClampedArray"), Color_reverse, "pixelData");
    }
//}@dev
...
function Color_effect(pixelData, // @arg Uint8ClampedArray - pixel data. [<R, G, B, A>, ...]
                      hslA) {    // @arg Float32Array|NumberArray - effect values. [<h, s, l, A>] [-359..359, -1..1, -1..1, 0..255]
                                 // @ret Uint8ClampedArray - new pixel data. [<R, G, B, A>, ...]
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(pixelData, "Uint8ClampedArray"),        Color_effect, "pixelData");
...
function Color_sepia(pixelData, // @arg Uint8ClampedArray - pixel data. [<R, G, B, A>, ...]
                     cb,        // @arg Number = -0.0911
                     cr) {      // @arg Number =  0.056
                                // @ret Uint8ClampedArray - pixel data
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(pixelData, "Uint8ClampedArray"), Color_sepia, "pixelData");
...
function Color_gray(pixelData) { // @arg Uint8ClampedArray - pixel data. [<R, G, B, A>, ...]
                                 // @ret Uint8ClampedArray - pixel data. [<R, G, B, A>, ...]
//{@dev
    if (!global["BENCHMARK"]) {
        $valid($type(pixelData, "Uint8ClampedArray"), Color_gray, "pixelData");
    }
//}@dev
